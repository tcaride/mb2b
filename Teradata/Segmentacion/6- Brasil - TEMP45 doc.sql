...
DROP TABLE TEMP_45.LK_account_money_doc_mlb;
CREATE TABLE TEMP_45.LK_account_money_doc_mlb as (
SELECT
  ...
  CASE WHEN v.VENTAS_USD is null or v.VENTAS_USD=0 or ((v.VENTAS_USD*5)/365) =0 then 'a.No Vende'
  WHEN am.balance/((v.VENTAS_USD*5)/365) <= 1 THEN 'b.Menos d lo que vende'
  WHEN am.balance/((v.VENTAS_USD*5)/365) <= 2 THEN 'c.Menos q el doble de lo que vende'
  WHEN am.balance/((v.VENTAS_USD*5)/365)  <= 5 THEN 'd.Hasta x 5 lo que vende'
  WHEN am.balance/((v.VENTAS_USD*5)/365) <= 20 THEN 'e.Hasta x 20 lo que vende'
  ELSE 'f.Mas de x 20 lo que vende' END as Ratio_AM_VTAS,

 CASE WHEN am.balance is null or am.balance=0 THEN 'No tiene AM'
  WHEN  am.balance< (20*5) THEN  'Menos 20 USD'
  WHEN am.balance<= (100*5)  THEN '20 a 100 USD'
  WHEN am.balance<= (300*5) THEN '100 a 300 USD'
  WHEN am.balance<= (1000*5)  THEN '300 a 1000 USD'
  WHEN am.balance<= (3000*5) THEN  '1000 a 3000 USD'
  WHEN am.balance<= (1000*5) THEN '3000 a 10000 USD'
...
DROP TABLE TEMP_45.segmentacion_doc_mlb;
CREATE  TABLE TEMP_45.segmentacion_doc_mlb  AS (
...
LEFT JOIN TEMP_45.LK_account_money_doc_mlb g ON a.b2b_id=g.b2b_id
