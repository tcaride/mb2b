...
----------------------------- Traigo rubro ------------------------------------------
  ...
  DROP TABLE TEMP_45.lk_lastmcc3;
...
----------------------------- Agrupo el rubro ------------------------------------------
  ...
  DROP TABLE TEMP_45.LK_lastmcc4;
...
----------------------------- Traigo datos de creditos por cust ------------------------------------------

  DROP TABLE TEMP_45.LK_credits_cust ;
  CREATE TABLE TEMP_45.LK_credits_cust as (
  SELECT
...
----------------------------- Traigo datos de seguros por cust ------------------------------------------

  DROP TABLE TEMP_45.Lk_seguros_cust ;
  CREATE TABLE TEMP_45.Lk_seguros_cust as (
  SELECT
    CUS_CUST_ID_buy,
    COUNT(*) total
...
----------------------------- 18. Trae datos de shipping ------------------------------------------
  ...
  CREATE TABLE temp_45.lk_seller_shipping_cust AS (
  SELECT
    bid.sit_site_id,
    bid.cus_cust_id_sel cus_cust_id_sel,
    COUNT(*) total
  ...
  LEFT JOIN WHOWNER.BT_SHP_SHIPMENTS AS shp
  on bid.shp_shipment_id = shp.shp_shipment_id AND shp.sit_site_id = bid.sit_site_id
  WHERE bid.sit_site_id IN ('MLA','MLM','MLC','MLB')--- ('MCO', 'MLA', 'MLB', 'MLC', 'MLM', 'MLU', 'MPE')
  AND bid.photo_id = 'TODATE'
  AND bid.tim_day_winning_date BETWEEN DATE '2020-10-01' AND DATE '2020-12-31' --OR bid.tim_day_winning_date between DATE '2019-01-01' and '2019-10-31')
  AND bid.ite_gmv_flag = 1
  AND bid.mkt_marketplace_id = 'TM'
