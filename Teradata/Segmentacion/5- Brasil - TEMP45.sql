...
CREATE TABLE TEMP_45.sell00_cust_mlm  AS (
SELECT
...
CREATE TABLE TEMP_45.sell01_cust_mlm as (
  SELECT   
  cus_cust_id_sel,
  sit_site_id,
  ...
  VENTAS_USD,
  Q
FROM TEMP_45.sell00_cust_mlm
)WITH data primary index (CUS_CUST_ID_SEL,SIT_SITE_ID);

-- traigo canal max
CREATE TABLE TEMP_45.sell02_cust_mlm as (
SELECT
  cus_cust_id_sel,
  sit_site_id,
  Canal
FROM TEMP_45.sell01_cust_mlm
group by 1,2,3
qualify row_number () over (partition by cus_cust_id_sel, sit_site_id order by SUM(VENTAS_USD) DESC) = 1
)WITH data primary index (CUS_CUST_ID_SEL,SIT_SITE_ID);

-- traigo segment detail max
CREATE TABLE TEMP_45.sell03_cust_mlm as (
SELECT
  cus_cust_id_sel,
  sit_site_id,
  tpv_segment_detail
...
-- traigo segment detail max
CREATE TABLE TEMP_45.sell04_cust_mlm as (
SELECT
  a01.cus_cust_id_sel,
  a01.sit_site_id,
  a02.canal canal_max,
...
FROM TEMP_45.sell01_cust_mlm a01
left join TEMP_45.sell02_cust_mlm a02
on a01.cus_cust_id_sel=a02.cus_cust_id_sel
left join TEMP_45.sell03_cust_mlm a03
...
CREATE TABLE TEMP_45.sell05_cust_mlm as (
  select
  a.cus_cust_id_sel,
  a.sit_site_id,
  b.SEGMENTO,
  ...
  a.cant_ventas,
  a.q_seg
FROM TEMP_45.sell04_cust_mlm a
...
CREATE TABLE TEMP_45.sell06_cust_mlm as (
  select
  a.cus_cust_id_sel,
  a.sit_site_id,
  b.kyc_entity_type,
  ...
  a.cant_ventas,
  a.q_seg
FROM TEMP_45.sell05_cust_mlm a
...
CREATE TABLE TEMP_45.sell07_cust_mlm as (
  select
  a.cus_cust_id_sel,
  a.sit_site_id,
  a.kyc_entity_type,
  ...
  a.cant_ventas,
  a.q_seg
FROM TEMP_45.sell06_cust_mlm a
...
CREATE TABLE temp_45.buy00_cust_mlm AS ( --- compras en el marketplace por empresa
SELECT
    bid.cus_cust_id_buy,  
    bid.sit_site_id,
...
CREATE TABLE TEMP_45.buy01_cust_mlm AS ( --- compras en el marketplace por usuario 
SELECT
    bid.cus_cust_id_buy,  
    bid.sit_site_id,
...
CREATE TABLE TEMP_45.buy02_cust_mlm AS (
SELECT
    tcb.cus_cust_id_buy,  
    tcb.sit_site_id,
...
FROM TEMP_45.buy01_cust_mlm tcb
...
CREATE TABLE TEMP_45.account_money2_mlm as (
SELECT
...
FROM TEMP_45.sell07_cust_mlm AS V
...
SELECT
  ...
  when engagement_rank = 1 and Agg_Frecuencia= 'Frequent Buyer' then 'Ver casos'
  when Agg_Frecuencia='Buyer' and engagement_rank<> 1 then 'Aumentar Frecuencia'
  when engagement_rank=2 and Agg_Frecuencia= 'Frequent Buyer' then 'Sumar Servicios'
...
LEFT JOIN temp_45.sell07_cust_mlm AS b ON a.cus_cust_id=b.cus_cust_id_sel 
...
LEFT JOIN temp_45.buy00_cust_mlm AS f ON a.cus_cust_id=f.cus_cust_id_buy 
LEFT JOIN TEMP_45.account_money2_mlm g ON a.cus_cust_id=g.cus_cust_id
...
LEFT JOIN temp_45.buy02_cust_mlm o ON a.cus_cust_id=o.cus_cust_id_buy 
